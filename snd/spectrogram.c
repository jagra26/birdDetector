...
static void get_colour_map_value(float value, double spec_floor_db,
                                 unsigned char colour[3], bool gray_scale) {
  ...
  if (gray_scale) { /* "value" is a negative value in decibels.
...
static void read_mono_audio(SNDFILE *file, sf_count_t filelen, double *data,
                            int datalen, int indx, int total) {
  sf_count_t start;
...
static void render_spectrogram(cairo_surface_t *surface, double spec_floor_db,
                               float **mag2d, double maxval, double left,
  ...
  for (w = 0; w < width; w++)
    for (h = 0; h < height; h++) {
      ...
      mag2d[w][h] = (mag2d[w][h] < linear_spec_floor)
                        ? spec_floor_db
                        : 20.0 * log10(mag2d[w][h]);
...
static void render_heat_map(cairo_surface_t *surface, double magfloor,
                            const RECT *r, bool gray_scale) {
  ...
  for (h = 0; h < r->height; h++) {
    get_colour_map_value(magfloor * (r->height - h) / (r->height + 1), magfloor,
                         colour, gray_scale);
...
static int calculate_ticks(double min, double max, double distance,
                           int log_scale, TICKS *ticks) {
...
#define add_tick(val, just_a_tick)                                             \
  do {                                                                         \
    if (val >= min - DELTA && val < max + DELTA) {                             \
      ...
      ticks->distance[k] =                                                     \
          distance * (log_scale == 2 ? /*log*/ (log(val) - log(min)) /         \
                                           (log(max) - log(min))               \
                                     : /*lin*/ (val - min) / range);           \
      k++;                                                                     \
    };                                                                         \
  ...
  while (value <= max + DELTA) { /* Add a tick next to each printed number */
...
*/
static int add_log_ticks(double min, double max, double distance, TICKS *ticks,
  ...
  for (value = start_value; value <= max + DELTA; value *= 10.0) {
    ...
    ticks->distance[k] =
        distance * (log(value) - log(min)) / (log(max) - log(min));
    k++;
  };
  return k;
} /* add_log_ticks */
...
static int calculate_log_ticks(double min, double max, double distance,
  ...
  */
  ...
  if (k >= TARGET_DIVISIONS + 1) /* Number of labels is n.of divisions + 1 */
  {
    ...
    for (i = 2; i <= 9; i++)
      k = add_log_ticks(min, max, distance, ticks, k, underpinning * (1.0 * i),
                        i == 2 || i == 5);
  };
...
static void str_print_value(char *text, int text_len, double value,
  ...
  if (fabs(value) < 1e-10)
    ...
    snprintf(text, text_len, "%.*f", decimal_places_to_print, value);
} /* str_print_value */

static void render_spect_border(cairo_surface_t *surface, const char *filename,
                                double left, double width, double seconds,
                                double top, double height, double min_freq,
                                double max_freq, bool log_freq) {
  char text[512];
  cairo_t *cr;
  ...
  for (k = 0; k < tick_count; k++) {
    ...
    cairo_text_extents(cr, text, &extents);
    cairo_move_to(cr, left + ticks.distance[k] - extents.width / 2,
                  top + height + 8 + extents.height);
    cairo_show_text(cr, text);
  };
  ...
  for (k = 0; k < tick_count; k++) {
    ...
    str_print_value(text, sizeof(text), ticks.value[k],
                    ticks.decimal_places_to_print);
    cairo_text_extents(cr, text, &extents);
    cairo_move_to(cr, left + width + 12,
                  top + height - ticks.distance[k] + extents.height / 4.5);
    cairo_show_text(cr, text);
  };
  ...
  cairo_text_extents(cr, text, &extents);
  cairo_move_to(cr, left + (width - extents.width) / 2,
                cairo_image_surface_get_height(surface) - 8);
  cairo_show_text(cr, text);
  ...
  cairo_move_to(cr, cairo_image_surface_get_width(surface) - 12,
                top + (height + extents.width) / 2);
  cairo_show_text(cr, text);

  cairo_destroy(cr);
} /* render_spect_border */

static void render_heat_border(cairo_surface_t *surface, double magfloor,
                               const RECT *r) {
  ...
  for (k = 0; k < tick_count; k++) {
    ...
    str_print_value(text, sizeof(text), -1.0 * ticks.value[k],
                    ticks.decimal_places_to_print);
    cairo_text_extents(cr, text, &extents);
    cairo_move_to(cr, r->left + r->width + 2 * TICK_LEN,
                  r->top + ticks.distance[k] + extents.height / 4.5);
    cairo_show_text(cr, text);
  };
...
*/
static double magindex_to_specindex(int speclen, int maglen, int magindex,
...
static void interp_spec(float *mag, int maglen, const double *spec, int speclen,
                        const RENDER *render, int samplerate) {
  int k;
  ...
  */
  ...
  for (k = 0; k < maglen;
       k++) { /* Average the pixels in the range it comes from */
    double this =
        magindex_to_specindex(speclen, maglen, k, render->min_freq,
                              render->max_freq, samplerate, render->log_freq);
    double next =
        magindex_to_specindex(speclen, maglen, k + 1, render->min_freq,
                              render->max_freq, samplerate, render->log_freq);
    ...
    if (next >
        this + 1) { /* The output indices are more sparse than the input indices
                    ** so average the range of input indices that map to this
                    *output,
                    ** making sure not to exceed the input array (0..speclen
                    *inclusive)
                    */
...
static void render_to_surface(const RENDER *render, SNDFILE *infile,
                              int samplerate, sf_count_t filelen,
  ...
  if (render->border) {
    width = lrint(cairo_image_surface_get_width(surface) - LEFT_BORDER -
                  RIGHT_BORDER);
    height = lrint(cairo_image_surface_get_height(surface) - TOP_BORDER -
                   BOTTOM_BORDER);
  } else {
    width = render->width;
  ...
  */
  ...
  /* Find the nearest fast value for the FFT size. */
  {
    ...
    for (d = 0; /* Will terminate */;
         d++) { /* Logarithmically, the integer above is closer than
  ...
  for (w = 0; w < width; w++) {
    ...
    interp_spec(mag_spec[w], height, spec->mag_spec, speclen, render,
                samplerate);
  };
  ...
  if (render->border) {
    ...
    render_spectrogram(surface, render->spec_floor_db, mag_spec, max_mag,
                       LEFT_BORDER, TOP_BORDER, width, height,
                       render->gray_scale);

    render_heat_map(surface, render->spec_floor_db, &heat_rect,
                    render->gray_scale);

    render_spect_border(surface, render->filename, LEFT_BORDER, width,
                        filelen / (1.0 * samplerate), TOP_BORDER, height,
                        render->min_freq, render->max_freq, render->log_freq);
    render_heat_border(surface, render->spec_floor_db, &heat_rect);
  } else
    render_spectrogram(surface, render->spec_floor_db, mag_spec, max_mag, 0, 0,
                       width, height, render->gray_scale);
...
static void render_cairo_surface(const RENDER *render, SNDFILE *infile,
                                 int samplerate, sf_count_t filelen) {
  cairo_surface_t *surface = NULL;
  cairo_status_t status;

  /*
  **	CAIRO_FORMAT_RGB24 	 each pixel is a 32-bit quantity, with the upper
  *8 bits *	unused. Red, Green, and Blue are stored in the remaining 24 bits
  *in that order.
  */

  surface = cairo_image_surface_create(CAIRO_FORMAT_RGB24, render->width,
                                       render->height);
  if (surface == NULL ||
...
static void render_sndfile(RENDER *render) {
  SNDFILE *infile;
  SF_INFO info = {};
...
static void usage_exit(const char *argv0, int error) {
  const char *progname;

  progname = strrchr(argv0, '/');
  progname = (progname == NULL) ? argv0 : progname + 1;

  printf("\nUsage :\n\n    %s [options] <sound file> <img width> <img height> "
         "<png name>\n\n",
         progname);

  puts("    Create a spectrogram as a PNG file from a given sound file. The\n"
       "    spectrogram image will be of the given width and height.\n");

  puts("    Options:\n"
       "        --dyn-range=<number>   : Dynamic range (default is 180 for "
       "180dB range)\n"
       "        --no-border            : Drop the border, scales, heat map and "
       "title\n"
       "        --min-freq=<number>    : Set the minimum frequency in the "
       "output\n"
       "        --max-freq=<number>    : Set the maximum frequency in the "
       "output\n"
       "        --fft-freq=<number>    : Set the lowest resolvable frequency "
       "and the\n"
       "                                 height of each band in the linear "
       "spectrogram.\n"
       "                                 Lower values increase frequency "
       "resolution but\n"
       "                                 smear the output horizontally and "
       "higher values\n"
       "                                 improve the temporal definition but "
       "decrease the\n"
       "                                 distinction between the lowest "
       "frequencies.\n"
       "        --log-freq             : Use a logarithmic frquency scale\n"
       "        --gray-scale           : Output gray pixels instead of a heat "
       "map\n"
       "        --kaiser               : Use a Kaiser window function (the "
       "default)\n"
...
int main(int argc, char *argv[]) {
  ...
  render.filename =
      (render.filename != NULL) ? render.filename + 1 : render.sndfilepath;
